...
<h1>PvP Tier List (Drag & Drop)</h1>

<div class="container">

  <div class="tier" ondrop="drop(event)" ondragover="allowDrop(event)">
    <h2>HT1</h2>
    <div class="players" id="HT1">
      <div class="player" draggable="true" ondragstart="drag(event)">Dream</div>
      <div class="player" draggable="true" ondragstart="drag(event)">Techno</div>
    </div>
  </div>

  <div class="tier" ondrop="drop(event)" ondragover="allowDrop(event)">
    <h2>HT2</h2>
    <div class="players" id="HT2">
      <div class="player" draggable="true" ondragstart="drag(event)">Player1</div>
      <div class="player" draggable="true" ondragstart="drag(event)">Player2</div>
    </div>
  </div>

  <div class="tier" ondrop="drop(event)" ondragover="allowDrop(event)">
    <h2>HT3</h2>
    <div class="players" id="HT3">
      <div class="player" draggable="true" ondragstart="drag(event)">Player3</div>
    </div>
  </div>
...
// load saved state
window.onload = () => {
  const data = localStorage.getItem("tiers");
  if (data) {
    document.getElementById("HT1").innerHTML = "";
    document.getElementById("HT2").innerHTML = "";
    document.getElementById("HT3").innerHTML = "";

    const saved = JSON.parse(data);

    for (let tier in saved) {
      saved[tier].forEach(p => {
        addPlayer(tier, p);
      });
    }
  }
...
function drop(ev) {
  ev.preventDefault();
  let container = ev.target.closest(".players");
...
function save() {
  ...
  document.querySelectorAll("#HT1 .player").forEach(p => data.HT1.push(p.innerText));
  document.querySelectorAll("#HT2 .player").forEach(p => data.HT2.push(p.innerText));
  document.querySelectorAll("#HT3 .player").forEach(p => data.HT3.push(p.innerText));
