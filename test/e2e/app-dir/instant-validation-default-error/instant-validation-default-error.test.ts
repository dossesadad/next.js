...
describe('instant validation - default error', () => {
  ...
  // Default validation level is 'error'. Implicit validation fires on bare
  // pages in dev AND build — error level applies to both modes. Per-segment
  // overrides (`level`, `true`, `false`) layer on top of this. A
  // `level: 'warning'` override de-escalates a specific route to dev-only.
  //
  ...
  if (isNextDev) {
    describe('dev', () => {
      it('bare page: implicit validation surfaces a redbox (default error fires)', async () => {
      ...
      it('explicit-error page: explicit override at default level, instant redbox in dev', async () => {
      ...
      it('explicit-true page: aliases to default error, instant redbox in dev', async () => {
    ...
    describe('build', () => {
      ...
      it('layered: bare page under layout-with-instant-false still fails the build', async () => {
        ...
        // fails because the default 'error' level applies to build.
