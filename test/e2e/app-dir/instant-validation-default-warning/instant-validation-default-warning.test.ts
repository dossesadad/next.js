...
describe('instant validation - default warning', () => {
  ...
  // Default validation level is 'warning'. Implicit validation fires on
  // bare pages in dev only — build does not validate unless a segment
  // explicitly escalates with `level: 'error'`. Per-segment overrides
  ...
  if (isNextDev) {
    describe('dev', () => {
      it('bare page: implicit validation surfaces a redbox (default warning fires)', async () => {
      ...
      it('explicit-error page: instant redbox surfaces in dev', async () => {
        ...
        await expect(browser).toDisplayCollapsedRedbox(`
         ...
         >  7 | export const unstable_instant = { level: 'error' as const }
              |                                 ^",
               "stack": [
      ...
      it('explicit-true page: aliases to default warning, instant redbox in dev', async () => {
      ...
      it('explicit-warning page: explicit override at default level, instant redbox in dev', async () => {
