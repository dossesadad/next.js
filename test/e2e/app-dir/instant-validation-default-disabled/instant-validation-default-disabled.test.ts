...
describe('instant validation - default disabled', () => {
  ...
  //   data at the top. Under default 'disabled', instant validation does
  //   not run, so violating bare pages produce no errors at all.
  //
  ...
  //   default 'disabled', instant validation still does not run, so the
  //   error visible to the user is a static-shell error — distinct from
  //   the instant validation error format.
  //
  // The contrast between the two trees demonstrates that bumping the
  // framework default in the future would not silently opt routes into
  // instant validation that hadn't asked for it.

  describe('with-root-suspense', () => {
    if (isNextDev) {
      describe('dev', () => {
        it('bare page: no errors (static shell satisfied, instant disabled)', async () => {
        ...
        it('explicit-error page: instant redbox surfaces despite disabled default', async () => {
          ...
          await expect(browser).toDisplayCollapsedRedbox(`
           ...
           >  8 | export const unstable_instant = { level: 'error' as const }
                |                                 ^",
                 "stack": [
    ...
    if (isNextDev) {
      describe('dev', () => {
        it('bare page: static-shell error surfaces but is not labelled Instant', async () => {
          ...
          // did not run under the disabled default.
          await expect(browser).toDisplayCollapsedRedbox(`
           {
             "code": "E1164",
             "description": "Next.js encountered uncached data during the initial render.",
      ...
      describe('build', () => {
        ...
        it('explicit-error page: build fails with static-shell error, not instant', async () => {
          // Even with `unstable_instant = { level: 'error' }`, the build
